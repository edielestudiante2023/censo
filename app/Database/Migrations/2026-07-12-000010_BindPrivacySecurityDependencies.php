...
class BindPrivacySecurityDependencies extends Migration
{
    public function up(): void
    {
        ...
        foreach ($documents as $document) {
            ...
            $content = (new PrivacyDocumentService())->render('seguridad', $client, $program, $bases, $purposes,
                ['third_parties' => $thirdParties, 'document_versions' => $versions]);
            $variables = json_decode((string) ($document['variables_json'] ?? '{}'), true) ?: [];
            $variables['security_schema_version'] = 2; $variables['document_dependencies'] = $dependencies;
    ...
    {
        // Las dependencias juridicas versionadas no se degradan automaticamente.
    }
}
